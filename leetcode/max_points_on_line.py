    ...
    def maxPoints(self, points):
        ...
        for i in range(pointLen):
            ...
            for j in range(i+1, pointLen):
                pEnd = points[j]
                if pStart.equal(pEnd):
                    continue
                (a, b, c)= (pStart.x-pEnd.x, pStart.y-pEnd.y,
                        pStart.y*pEnd.x - pStart.x*pEnd.y)
                gc = self._gcd(a, b)
                if a < 0:
                    a, b, c = -a, -b, -c
                lineParams = (a/gc, b/gc, c/gc)
                #print lineParams
                paramKey = '-'.join([str(lineParams[0]), str(lineParams[1]), str(lineParams[2])])
                if lineParamDict.has_key(paramKey):
                    #print 'match'
                    counter = lineParamDict[paramKey]
                else:
                    counter = 0
                    for pTest in points:
                        counter += 1 if (lineParams[0]*pTest.y - lineParams[1]*pTest.x + lineParams[2]) == 0 else 0
                    lineParamDict[paramKey] = counter
