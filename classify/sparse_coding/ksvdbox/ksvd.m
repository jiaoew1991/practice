...
for iter = 1:iternum
  ...
  if (printerr)
    info = sprintf('%s, %s = %.4g, mutual coherence:%.4g', info, errstr, err(iter),mutual_coherence(D));
  end
  if (printgerr)
...
if (exactsvd)
  ...
  for t = 1:maxit
      ...
      max_val = 0;
      sel_index = 0;
      for s = 1:length(atomcorra)
          if (abs(atomcorra(s))>max_val)
             sel_index = s;
             max_val = abs(atomcorra(s));
          end
      end
      atomdelta = tradeoff*sign(atomcorra(sel_index))*D(:,sel_index);
      atom = (atom - atomdelta);
      atom = atom/norm(atom);
  end
